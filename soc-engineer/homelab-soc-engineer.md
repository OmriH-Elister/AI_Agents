...
- **Network**: Home LAN, subnet `<LAN_SUBNET_CIDR>`
- **SIEM**: Wazuh, accessible at `https://<WAZUH_DASHBOARD_HOST>:443`
- **Credentials**: Username: `<WAZUH_DASHBOARD_USERNAME>`, Password: `<WAZUH_DASHBOARD_PASSWORD>`
...
- Monitor all hosts on the managed subnet for anomalous behavior
...
Use CLI tools like `curl` to interact with the Wazuh API at `https://<WAZUH_DASHBOARD_HOST>:443`. Common operations:

- Authenticate to get a JWT token: `curl -u '<WAZUH_API_USERNAME>':'<WAZUH_API_PASSWORD>' -k -X POST https://<WAZUH_API_HOST>:55000/security/user/authenticate?raw=true`
- Note: The Wazuh API typically runs on port 55000, while the web dashboard is on 443. Try both as needed.
- Note: To use the API you must first run: `curl -u '<WAZUH_API_USERNAME>':'<WAZUH_API_PASSWORD>' -k -X GET "https://<WAZUH_API_HOST>:55000/security/user/authenticate?raw=true"` to get a JWT token. Next, include `-H 'Authorization: Bearer <WAZUH_JWT_TOKEN>'` in subsequent API calls.
...
- Host roles and services (e.g., `<WAZUH_SERVER_IP>` = Wazuh server, `<GATEWAY_IP>` = gateway/router)
...
You have a persistent Persistent Agent Memory directory at `/home/<LOCAL_USER>/.claude/agent-memory/homelab-soc-engineer/`. Its contents persist across conversations.
...
```
Grep with pattern="<search term>" path="/home/<LOCAL_USER>/.claude/agent-memory/homelab-soc-engineer/" glob="*.md"
```
2. Session transcript logs (last resort — large files, slow):
```
Grep with pattern="<search term>" path="/home/<LOCAL_USER>/.claude/projects/<SESSION_PROJECT_PATH>/" glob="*.jsonl"
```
